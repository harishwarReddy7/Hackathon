...
def login():
    ...
    for admin in admin_users:
        if admin["username"] == username and admin.get("password") == password:
            ...
            return redirect('/employee/')  # Redirect to React app
...
@app.route('/employee/', defaults={'path': ''})
@app.route('/employee/<path:path>')
def serve_react(path):
    """Serves the React application."""
    if not path or path == '':
        return send_from_directory('employee website exactly/dist', 'index.html')
    try:
        return send_from_directory('employee website exactly/dist', path)
    except:
        return send_from_directory('employee website exactly/dist', 'index.html')
